...
with open('base.json') as b:
    labels = [l.split('.')[1] for l in json.load(b)['label_names']]
    labels = [labels[i] for i in range(2, len(labels), 4)]
...
for k, v in hier.items():
    ...
    if k not in labels:
        labels.append(k)
    am[labels.index(k)][66] = 1
    am[66][labels.index(k)] = 1
...
dists = csgraph.shortest_path(csr_matrix(am), directed=False)[:len(tc),:len(tc)]
print(dists.mean())
